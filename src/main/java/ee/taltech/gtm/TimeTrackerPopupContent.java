...
final class TimeTrackerPopupContent extends Box {
	...
	TimeTrackerPopupContent(@NotNull TimeTrackerService service, @NotNull Consumer<PatternField> patternFieldFocusListener) {
...

		{
			...
			final JSpinner idleThresholdSpinner = new JSpinner(new SpinnerNumberModel(Util.msToS(service.getIdleThresholdMs()), 0, Integer.MAX_VALUE, 10));
...

		{
			...
			loadDefaults.addActionListener(e1 -> {
				service.loadStateDefaults(ee.taltech.gtm.DefaultSettingsService.getDefaultState());
				popup.cancel();
			});
			...
			saveDefaults.addActionListener(e1 -> {
				ee.taltech.gtm.DefaultSettingsService.setDefaultState(service.getState());
				popup.cancel();
			});
	...
	private static final class TimePatternTextField extends Box {
		...
		private final ArrayList<ee.taltech.gtm.TimePattern.ParseError> errors = new ArrayList<>();

		private final Consumer<ee.taltech.gtm.TimePattern> onChanged;
		...
		public TimePatternTextField(String content, Consumer<ee.taltech.gtm.TimePattern> onChanged, Consumer<PatternField> focusListener, PatternField patternFieldType) {
			...
			errorButton.addActionListener(e -> {
				...
				final ee.taltech.gtm.TimePattern.ParseError error = errors.get(shownErrorIndex);
		...
		private void refresh(String pattern) {
			errors.clear();
			final ee.taltech.gtm.TimePattern result = ee.taltech.gtm.TimePattern.parse(pattern, errors);
			if (onChanged != null) {
