...
import static ee.taltech.gtm.TimeTrackingStatus.RUNNING;
...
public final class TimeTrackerWidget extends JButton implements CustomStatusBarWidget {
    ...
    public static final String ID = "ee.taltech.gtm.DarkyenusTimeTracker";
    ...
    private void popupSettings() {
        final ee.taltech.gtm.TimeTrackerPopupContent content = new ee.taltech.gtm.TimeTrackerPopupContent(service, (patternFieldType) -> {
            if (patternFieldType == null) {
                setPopupState(PopupState.VISIBLE_LOST_FOCUS);
            } else if (patternFieldType == ee.taltech.gtm.TimeTrackerPopupContent.PatternField.WIDGET) {
                setPopupState(PopupState.VISIBLE_WIDGET_PATTERN_FOCUS);
            } else if (patternFieldType == ee.taltech.gtm.TimeTrackerPopupContent.PatternField.GIT) {
    ...
    private ee.taltech.gtm.TimePattern currentShowTimePattern() {
    ...
    public void paintComponent(final Graphics g) {
        ...
        final ee.taltech.gtm.TimeTrackingStatus status = service.getStatus();
        if (mouseInside) {
            if (status == RUNNING) {
    ...
    private ee.taltech.gtm.TimePattern getPreferredSize_lastPattern = null;
    ...
    public Dimension getPreferredSize() {
        ...
        final ee.taltech.gtm.TimePattern pattern = currentShowTimePattern();
    ...
    public static final ee.taltech.gtm.TimePattern FULL_TIME_FORMATTING = ee.taltech.gtm.TimePattern.parse("{{lw \"week\"s}} {{ld \"day\"s}} {{lh \"hour\"s}} {{lm \"minute\"s}} {{s \"second\"s}}");
