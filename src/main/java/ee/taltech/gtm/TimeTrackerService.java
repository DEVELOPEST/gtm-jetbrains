...
import static ee.taltech.gtm.GitIntegration.SetupCommitHookResult.GIT_DIR_NOT_FOUND;
import static ee.taltech.gtm.GitIntegration.SetupCommitHookResult.GIT_HOOKS_DIR_NOT_FOUND;
...
public final class TimeTrackerService implements PersistentStateComponent<TimeTrackerPersistentState>, Disposable {
	...
	@Nullable
	private ee.taltech.gtm.GitIntegration gitIntegrationComponent;

	@Nullable
	private ee.taltech.gtm.TimeTrackerWidget widget;

	private long totalTimeMs = 0;
	private volatile ee.taltech.gtm.TimeTrackingStatus status = ee.taltech.gtm.TimeTrackingStatus.STOPPED;
	...
	@Nullable
	private ee.taltech.gtm.TimePattern ideTimePattern;
	@Nullable
	private ee.taltech.gtm.TimePattern gitTimePattern;
	...
	public TimeTrackerService(@NotNull Project project) {
		...
		EditorFactory.getInstance().getEventMulticaster().addDocumentListener(new DocumentListener() {
			...
			public void documentChanged(@NotNull DocumentEvent e) {
				if (!isAutoStart() || getStatus() == ee.taltech.gtm.TimeTrackingStatus.RUNNING) return;
				...
				EventQueue.invokeLater(() -> {
					...
					if(e.getDocument().equals(selectedTextEditor.getDocument())) {
						setStatus(ee.taltech.gtm.TimeTrackingStatus.RUNNING);
					}
				});
			}
		}, this);

		ee.taltech.gtm.InactivityService.getInstance().initListening(project, this);
	}

	@NotNull
	public ee.taltech.gtm.TimeTrackerWidget widget() {
		ee.taltech.gtm.TimeTrackerWidget widget;
		synchronized (this) {
			widget = this.widget;
			if (widget == null) {
				this.widget = widget = new ee.taltech.gtm.TimeTrackerWidget(this);
			}
		}
		return widget;
	}
	...
	@NotNull
	public ee.taltech.gtm.TimeTrackingStatus getStatus() {
		return status;
	}
	...
	public synchronized void toggleRunning() {
		switch (this.status) {
			case RUNNING:
				setStatus(ee.taltech.gtm.TimeTrackingStatus.STOPPED);
				break;
			case STOPPED:
			case IDLE:
				setStatus(ee.taltech.gtm.TimeTrackingStatus.RUNNING);
	...
	private synchronized void tick() {
		if (status != ee.taltech.gtm.TimeTrackingStatus.RUNNING) {
		...
		if (sinceLastTickMs > TICK_JUMP_DETECTION_THRESHOLD_MS) {
			...
			setStatus(stopWhenIdleRatherThanPausing ? ee.taltech.gtm.TimeTrackingStatus.STOPPED : ee.taltech.gtm.TimeTrackingStatus.IDLE, lastValidTimeMs);
	...
	private synchronized void otherComponentStarted() {
		if (status != ee.taltech.gtm.TimeTrackingStatus.STOPPED) {
			setStatus(ee.taltech.gtm.TimeTrackingStatus.IDLE);
		}
	}
	...
	private synchronized void addTotalTimeMs(long milliseconds) {
		...
		updateGitTime(Util.msToS(milliseconds));
	}
	...
	public synchronized void setStatus(@NotNull ee.taltech.gtm.TimeTrackingStatus status) {
		setStatus(status, System.currentTimeMillis());
	}

	private void setStatus(final @NotNull ee.taltech.gtm.TimeTrackingStatus status, final long now) {
		...
		switch (this.status) {
			...
			case IDLE: {
				if (Util.msToS(msInState) <= autoCountIdleSeconds) {
					addTotalTimeMs(msInState);
	...
	public synchronized int getTotalTimeSeconds() {
		...
		if (this.status == ee.taltech.gtm.TimeTrackingStatus.RUNNING) {
			final long now = System.currentTimeMillis();
			resultMs += Math.max(0L, now - statusStartedMs);
		}

		return (int) Util.msToS(resultMs);
	}
	...
	public synchronized Boolean setGitIntegration(boolean enable) {
		final Path projectBase;
		{
			...
			projectBase = Util.convertToIOFile(Util.getProjectBaseDir(project));
			if (projectBase == null) {
				if (enable) {
		...
		ee.taltech.gtm.GitIntegration gitIntegrationComponent = this.gitIntegrationComponent;
		if (gitIntegrationComponent == null) {
			...
			gitIntegrationComponent = this.gitIntegrationComponent = new ee.taltech.gtm.GitIntegration(gitDir, gitHooksDir);
		}

		final ee.taltech.gtm.GitIntegration.SetupCommitHookResult result = gitIntegrationComponent.setupCommitHook(enable);
		switch (result) {
			case SUCCESS: {
				if (enable) {
					updateGitTime(0);
					...
					public void actionPerformed(@NotNull AnActionEvent e) {
						...
						ApplicationManager.getApplication().invokeLater(() -> {
							...
							final FileChooserDescriptor fileChooserDescriptor = new FileChooserDescriptor(false, true, false, false, false, false)
									...
									.withRoots(Util.getProjectBaseDir(project))
							...
							if (chosen.length >= 1) {
								chosenPath = Util.convertToIOFile(chosen[0]);
							}
							if (chosenPath == null) {
	...
	private void nagAboutGitIntegrationIfNeeded() {
		if ((naggedAbout & TimeTrackerPersistentState.NAGGED_ABOUT_GIT_INTEGRATION) == 0 && !gitIntegration) {
			ApplicationManager.getApplication().invokeLater(() -> {
				final VirtualFile projectBaseDir = Util.getProjectBaseDir(project());
	...
	@NotNull
	public ee.taltech.gtm.TimePattern getIdeTimePattern() {
		// widget sometimes manages to ask for the pattern before initialization, so we return dummy value
		final ee.taltech.gtm.TimePattern pattern = this.ideTimePattern;
		return pattern == null ? NOTIFICATION_TIME_FORMATTING : pattern;
	}

	public synchronized void setIdeTimePattern(@NotNull ee.taltech.gtm.TimePattern ideTimePattern) {
		this.ideTimePattern = ideTimePattern;
		repaintWidget(true);
	}

	@NotNull
	public ee.taltech.gtm.TimePattern getGitTimePattern() {
		final ee.taltech.gtm.TimePattern pattern = this.gitTimePattern;
		return pattern == null ? NOTIFICATION_TIME_FORMATTING : pattern;
	}

	public synchronized void setGitTimePattern(@NotNull ee.taltech.gtm.TimePattern gitTimePattern) {
		this.gitTimePattern = gitTimePattern;
		updateGitTime(0);
	}

	private void updateGitTime(long seconds) {
		final ee.taltech.gtm.GitIntegration gitIntegrationComponent = this.gitIntegrationComponent;
		final ee.taltech.gtm.TimePattern gitTimePattern = this.gitTimePattern;
	...
	private void repaintWidget(boolean relayout) {
		final ee.taltech.gtm.TimeTrackerWidget widget = this.widget;
		if (widget != null) {
	...
	public void noStateLoaded() {
		...
		loadState(ee.taltech.gtm.DefaultSettingsService.getDefaultState());
	}
	...
	public void loadState(@NotNull TimeTrackerPersistentState state) {
		...
		ApplicationManager.getApplication().invokeLater(() -> {
			synchronized (this) {
				...
				setIdeTimePattern(ee.taltech.gtm.TimePattern.parse(state.ideTimePattern));

				gitIntegration = false;//Otherwise setGitTimePattern triggers update
				setGitTimePattern(ee.taltech.gtm.TimePattern.parse(state.gitTimePattern));
	...
	public synchronized TimeTrackerPersistentState getState() {
		...
		result.totalTimeSeconds = Util.msToS(totalTimeMs);
	...
	public void notifyUserNotIdle() {
		final long now = System.currentTimeMillis();
		this.lastActivityMs = now;
		if (status == ee.taltech.gtm.TimeTrackingStatus.IDLE) {
			synchronized (this) {
				if (status == ee.taltech.gtm.TimeTrackingStatus.IDLE) {
					setStatus(ee.taltech.gtm.TimeTrackingStatus.RUNNING, now);
				}
			}
		}
	}
